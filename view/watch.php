<?php include 'view/inc/header.php';
$movie_poster_path = $movieInfo['poster_path'];

$movieGenres = [];
foreach ($movieInfo['genres'] as $genreArray) {
    foreach ($genreArray as $key => $value) {
        if ($key == "name") {
            array_push($movieGenres, $value);
        }
    }
}

?>
<div id="videoField">
    <?= "<iframe src='$videospider_url' width='800' height='500' allowfullscreen='true'></iframe>"; ?>
</div>

<div id="watch-movie-info" class="container">
    <div class="row>">
        <div class="watch-movie-detail-section col-12">
            <div class="row">
                <div class="watch-movie-image col-12 col-md-4 col-lg-3 col-xl-2">
                    <?php
                    echo "<img src='http://image.tmdb.org/t/p/w185$movie_poster_path' alt=''/>"
                    ?>
                </div>

                <div class="watch-movie-detail-info col-12 col-md-8 col-lg-9 col-xl-10">
                    <div class="watch-movie-title">
                        <h1><?= $movieInfo['original_title'] ?></h1>
                    </div>

                    <div class="watch-movie-details">
                        <table>
                            <tr>
                                <th>
                                    rating:
                                </th>
                                <td>
                                    <i class="fa fa-star"
                                       aria-hidden="true"></i> <?= number_format($movieInfo['vote_average'], 1, '.', '') ?>
                                    <span>(<?= $movieInfo['vote_count'] ?> votes)</span>
                                </td>
                            </tr>
                            <tr>
                                <th>release date:</th>
                                <td><?= date_create($movieInfo['release_date'])->format("d M Y"); ?></td>
                            </tr>
                            <tr>
                                <th>duration:</th>
                                <td><?= $movieInfo['runtime']; ?> min.</td>
                            </tr>
                            <tr>
                                <th>genres:</th>
                                <td><?= implode(", ", $movieGenres); ?></td>
                            </tr>
                            <tr>
                                <th>status:</th>
                                <td><?= $movieInfo['status']; ?></td>
                            </tr>
                            <tr>
                                <th>language:</th>
                                <td> <?= $movieInfo['original_language']; ?></td>
                            </tr>
                        </table>
                    </div>
                </div>

                <div class="watch-movie-overview col-12">
                    <h3>Overview</h3>
                    <p><?= $movieInfo['overview'] ?></p>
                </div>

                <div class="watch-similar-movies col-12">
                    <div class="row">
                        <div class="watch-similar-title col-12">
                            <h3>Similar movies:</h3>
                        </div>
                        <?= $similarMovies ?>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>

<?php include 'view/inc/footer.php'; ?>
